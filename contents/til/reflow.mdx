...
이번 글은 리플로우(레이아웃)에 관한 내용으로 다른 과정을 생략한다.

# 리플로우(레이아웃)

초기 브라우저가 웹 페이지의 뷰포트, 요소들의 크기나 높이와 같은 기하학적 계산을 하는 과정을 레이아웃이라 하며,  
웹 페이지의 기하학적 변화가 발생했을 때 다시 측정하는 것을 리플로우라 한다.  
...
<script>
  ...
  document.getElementById('add').onclick = () => {
    ...
    list.appendChild(li) // 컨테이너 높이 증가 → 주변 레이아웃 리플로우
  }
...
읽기 → 쓰기 순서가 메인 스레드에서 강제 레이아웃을 트리거
...
<script>
  document.getElementById('run').onclick = () => {
    ...
    // 레이아웃 값을 읽는 순간 브라우저는 최신 레이아웃을 보장하기 위해 계산을 수행할 수 있음
    const w = box.offsetWidth // 강제 레이아웃 가능
    // 곧바로 쓰기: 기하 변경 → 리플로우
...
리플로우가 비동기적으로 수행된다.
이 과정에서 브라우저는 스타일 및 레이아웃 계산을 지연하거나 배치하여
성능 영향을 최소화한다.

반면 4번은 JavaScript 실행 중 레이아웃 값을 즉시 요구하는 방식으로,
